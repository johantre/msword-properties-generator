...
class TestUtilsDownload(unittest.TestCase):
    ...
    @patch('msword_properties_generator.utils.utils_download.requests.get')
    def test_download_image_dropbox(self, mock_get):
        mock_response = MagicMock()
        mock_response.content = b'test content'
        mock_get.return_value = mock_response

        test_file = os.path.join(self.test_dir, 'test.jpg')
        download_image('https://www.dropbox.com/s/1234567890/test.jpg?dl=0', test_file)

        # Verify dl=0 was replaced with dl=1
        called_url = mock_get.call_args[0][0]
        self.assertIn('dl=1', called_url)
        self.assertNotIn('dl=0', called_url)
